...
file_name = os.path.join('logs', f"{dt_string}_log.txt")  # Bestand maken in 'logs' subfolder
...
def log(message):
    ct = datetime.datetime.now()
    with open(file_name, "a") as filehandle:  # "a" voor append mode
        filehandle.write(f"{ct.strftime('%Y-%m-%d %H:%M:%S')} - {message}\n")
def close_log():
    ct = datetime.datetime.now()
    with open(file_name, "a") as filehandle:  # "a" voor append mode
        filehandle.write("Log gesloten op: " + ct.strftime("%Y-%m-%d %H:%M:%S") + "\n")
    exit()
